...
class NaiveKMeans(Algorithm):

    def __init__(self):
        super().__init__()
        pass

    def planRoute(self, points, drone): #maximize information gain by choosing the next k cluster with the highest reward
        ...
        cluster_rewards = [] #[reward for cluster, cluster number]
        ...
        for i in range(k):
            ...
            cluster_rewards.append([totalClusterInformation, i + 1])
        cluster_rewards.sort(key=lambda x: x[0], reverse=True)
        print("Cluster Rewards: ", cluster_rewards)
        print("Centroids: ", centroids)

        #now pick the next highest cluster, travel to the centroid, and traverse as many points in order as you can.
        informationGain = 0
        while(len(cluster_rewards) > 0):
            nextCluster = cluster_rewards[0] # [reward, cluster number] list
